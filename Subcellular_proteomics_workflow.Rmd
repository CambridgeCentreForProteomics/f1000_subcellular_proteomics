...
The table below outlines how the `File.ID`s correspond to the raw MS files used 
...
information. To do so, we generate a `data.frame` with a column called `Cluster`
containing the cluster number and make sure that the `rownames` match the 
`featureNames` of our `unstim_msn` object. We also define our own colours to 
match the default colours used by `plot2D` such that our clusters have the same
colour annotations in our heatmap and dimensionality reduction visualisation.
Finally, we use the `pheatmap` function as above but pass our row annotation
`data.frame` to `annotation_row` and our pre-defined colours to `annotation_colors`.
...
`plot2D` function to generate PCA plots with the discovered clusters annotated. 
...
discovered clusters annotated. As before, we pass our `unstim_msn` object and
...
distribution profiles. We pass our `unstim_msn` object and subset the proteins
annotated as `"mitochondrion"` in the `markers` column of the `fData`.
...
visualisation and tell `plot2D` where the markers and results are located. For
example, to plot the markers we would pass `fcol = "markers"` and to plot the
...
We can now pass these coordinates to `plot2D` to visualise the data. Since they
...
results are only appended to the first `MSnSet` of each condition.
...
```{r bandle-violin1, out.width = '100%', echo = FALSE, fig.cap = "Violin plots displaying the BANDLE MCMC distribution of localisation probabilities across the subcellular niches in the data. Nine proteins are highlighted in the x-ray stimulated data that did not get asisgned a main localisation after thresholding."}
...
Source code and appendix located at GitHub [https://github.com/CambridgeCentreForProteomics/f1000_subcellular_proteomics](https://github.com/CambridgeCentreForProteomics/f1000_subcellular_proteomics). Files and appendix are also available from Zenodo under the 
identifier [doi:10.5281/zenodo.15100485](http://doi.org/10.5281/zenodo.15100485).
